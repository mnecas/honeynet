...
class HoneypotDeployment:
    def __init__(self, honeypot):
        ...
        self.honeynet_dir = os.path.join(self.honeynet_root_dir, str(honeypot.honeynet.id))
        ...
        self.honeypot_monitoring_path = os.path.join(self.honeypot_dir, "monitoring.yml")
    ...
    def up(self):
        ...
        result = subprocess.run(["deployment/deploy-honeypot.sh", self.honeypot_path, self.honeypot_monitoring_path])
    ...
    def down(self):
        ...
        if os.path.exists(self.honeypot_dir):
            result = subprocess.run(["deployment/delete-honeypot.sh", self.honeypot_path, self.honeypot_monitoring_path])
            if result.returncode != 0:
                return result
            shutil.rmtree(self.honeypot_dir)
